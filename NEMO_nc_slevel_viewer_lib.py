...
def Obs_reload_obs(var,Dataset_lst,tmp_current_time,ob_ti,Obs_dict,Obs_fname,Obs_JULD_datetime_dict,Obs_vis_d,Obs_varlst,Obs_reloadmeth):
    ...
    for tmp_datstr in Dataset_lst:
        ...
        for ob_var in Obs_var_lst_sub:
            ...
            ob_ti_diff = np.abs(Obs_noon_time_minus_current_time).min()

            ob_ti_curr_d_offset_threshold = 86400
            if len(Obs_noon_time_minus_current_time)>1:
                ob_ti_curr_d_offset_threshold = np.median(np.diff(Obs_noon_time_minus_current_time))

            if ob_ti_diff>ob_ti_curr_d_offset_threshold:
                ob_ti = None
            ...
            '''
            Obs_dat_dict[tmp_datstr][ob_var] = {}

            if ob_ti is None:
                Obs_dat_dict[tmp_datstr][ob_var]['loaded'] = False
            else:
                ...
                Obs_dat_dict[tmp_datstr][ob_var]['loaded'] = True
...
def obs_reset_sel(Dataset_lst, Fill = True): #,reset_datstr = None):
    ...
    obs_stat_id_sel,obs_stat_type_sel,obs_stat_time_sel,obs_load_sel = {},{},{},{}
    ...
    for tmp_datstr in Dataset_lst:
        ...
        obs_load_sel[tmp_datstr] = False
    
    return obs_z_sel,obs_obs_sel,obs_mod_sel,obs_lon_sel,obs_lat_sel,obs_stat_id_sel,obs_stat_type_sel,obs_stat_time_sel,obs_load_sel

"""
def obs_load_selected_point_old(secdataset_proc, Dataset_lst, Obs_var_lst_sub, Obs_dat_dict, obs_lon, obs_lat):
    #obs_z_sel,obs_obs_sel,obs_mod_sel,obs_lon_sel,obs_lat_sel,
    #                                        obs_stat_id_sel,obs_stat_type_sel,obs_stat_time_sel = obs_load_selected_point()
         
    (tmpobs_z_sel,tmpobs_obs_sel,tmpobs_mod_sel,tmpobs_lon_sel,tmpobs_lat_sel,
        tmpobs_stat_id_sel,tmpobs_stat_type_sel,tmpobs_stat_time_sel,tmpobs_load_sel) = obs_reset_sel(Dataset_lst, Fill = False)
    ...
    for ob_var in Obs_var_lst_sub:
        ...
        for tmp_datstr in Dataset_lst:
            ...
            tmpobs_dist_sel[ob_var][tmp_datstr] = np.ma.zeros((1))*np.ma.masked
...
def obs_load_selected_point_single(secdataset_proc, Dataset_lst, Obs_var_lst_sub, Obs_dat_dict, obs_lon, obs_lat):
    #obs_z_sel,obs_obs_sel,obs_mod_sel,obs_lon_sel,obs_lat_sel,
    #                                        obs_stat_id_sel,obs_stat_type_sel,obs_stat_time_sel = obs_load_selected_point()
...
#, 
    ...
    for ob_var in Obs_var_lst_sub:
        ...
        tmpobs_stat_time_sel[ob_var] = {}
        # and data sets.
        tmpobs_dist_sel[ob_var][secdataset_proc] = np.ma.masked
        ...
        if Obs_dat_dict[secdataset_proc][ob_var]['loaded']:
            ...
            tmpobs_obs = Obs_dat_dict[secdataset_proc][ob_var]['OBS'][:] 
            tmpobs_mod = Obs_dat_dict[secdataset_proc][ob_var]['MOD_HX'][:] 
            tmpobs_z = Obs_dat_dict[secdataset_proc][ob_var]['DEPTH'][:] 
            tmpobs_stat_id = Obs_dat_dict[secdataset_proc][ob_var]['STATION_IDENTIFIER'][:] 
            tmpobs_stat_type = Obs_dat_dict[secdataset_proc][ob_var]['STATION_TYPE'][:] 
            tmpobs_stat_time = Obs_dat_dict[secdataset_proc][ob_var]['JULD_datetime'][:] 
            tmpobs_z = Obs_dat_dict[secdataset_proc][ob_var]['DEPTH'][:] 
            ...
            #record the profile and distance for that Obs
            tmpobs_z_sel[ob_var][secdataset_proc] = tmpobs_z[tmp_obs_obs_ind]
            tmpobs_obs_sel[ob_var][secdataset_proc] = tmpobs_obs[tmp_obs_obs_ind]
            tmpobs_mod_sel[ob_var][secdataset_proc] = tmpobs_mod[tmp_obs_obs_ind]
            tmpobs_lon_sel[ob_var][secdataset_proc] = tmpobsx[tmp_obs_obs_ind]
            tmpobs_lat_sel[ob_var][secdataset_proc] = tmpobsy[tmp_obs_obs_ind]
            #pdb.set_trace()
            tmpobs_stat_id_sel[ob_var][secdataset_proc] = tmpobs_stat_id[tmp_obs_obs_ind].strip()
            tmpobs_stat_type_sel[ob_var][secdataset_proc] = tmpobs_stat_type[tmp_obs_obs_ind]
            tmpobs_stat_time_sel[ob_var][secdataset_proc] = tmpobs_stat_time[tmp_obs_obs_ind]

            tmpobs_dist_sel[ob_var][secdataset_proc] = tmp_obs_obs_dist
    #if Obs_dat_dict[secdataset_proc][ob_var]['loaded']:
    #pdb.set_trace()
    # put all distances into one array
    obs_dist_sel_cf_mat = np.ma.array([tmpobs_dist_sel[ob_var][secdataset_proc] for ob_var in Obs_var_lst_sub])
    '''
    obs_dist_sel_cf_lst = []
    for ob_var in Obs_var_lst_sub:
        if Obs_dat_dict[secdataset_proc][ob_var]['loaded']:
    ...
    #if  secdataset_proc == 'Dataset 2': pdb.set_trace()

    if ((obs_dist_sel_cf_mat).size>0):
        
        # select the Obs Obs type closest to the selected point
        if not obs_dist_sel_cf_mat.mask.all():
...
def obs_load_selected_point(secdataset_proc, Dataset_lst, Obs_var_lst_sub, Obs_dat_dict, obs_lon, obs_lat):
...
#, 
    ...
    for tmp_datstr in Dataset_lst: 
        ...
        for ob_var in Obs_var_lst_sub:
            if Obs_dat_dict[tmp_datstr][ob_var]['loaded']:
                obs_dist_sel_cf_lst.append(tmpobs_dist_sel[ob_var][tmp_datstr])
            else:
                obs_dist_sel_cf_lst.append(np.ma.masked)
        obs_dist_sel_cf_mat = np.ma.array(obs_dist_sel_cf_lst)
        '''
        if tmp_datstr in tmpobs_dist_sel[ob_var].keys():
            ...
            #pdb.set_trace()  
            #try:
            obs_dist_sel_cf_mat = np.ma.array([tmpobs_dist_sel[ob_var][tmp_datstr] for ob_var in Obs_var_lst_sub])
            #except:
            #    pdb.set_trace()
        '''
        '''
